...
import { generateSetmealDescription } from '@/api/ai'
...
export default class extends Vue {
  ...
  private async generateAiDescription() {
    if (!this.ruleForm.name || !this.ruleForm.name.trim()) {
      this.$message.warning('请先填写套餐名称')
      return
    }

    // 获取分类名
    const category = (this.setMealList as any[]).find(
      (item: any) => item.id === this.ruleForm.idType
    )
    const categoryName = category ? category.name : '中式'

    // 构建菜品列表信息
    let dishListInfo = ''
    if (this.dishTable && this.dishTable.length > 0) {
      dishListInfo = (this.dishTable as any[])
        .map((d: any) => d.name + ' x' + d.copies)
        .join('、')
    }

    this.aiGenerating = true
    try {
      const res = await generateSetmealDescription({
        setmealName: this.ruleForm.name,
        categoryName: categoryName,
        dishListInfo: dishListInfo
      })
      if (res && res.data && res.data.code === 1) {
        this.ruleForm.description = res.data.data
        this.$message.success('AI描述生成成功')
      } else {
        this.$message.error(res.data.msg || 'AI生成失败')
      }
    } catch (err) {
