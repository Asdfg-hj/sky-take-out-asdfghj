...
import { generateDishDescription } from '@/api/ai'
...
export default class extends Vue {
  ...
  private async generateAiDescription() {
    if (!this.ruleForm.name || !this.ruleForm.name.trim()) {
      this.$message.warning('请先填写菜品名称')
      return
    }

    // 获取分类名
    const category = (this.dishList as any[]).find(
      (item: any) => item.id === this.ruleForm.categoryId
    )
    const categoryName = category ? category.name : '中式'

    // 构建口味信息
    let flavorInfo = ''
    if (this.dishFlavors.length > 0) {
      flavorInfo = this.dishFlavors
        .filter((f: any) => f.name && f.value && f.value.length > 0)
        .map((f: any) => f.name + '可选（' + f.value.join('/') + '）')
        .join('、')
    }
